  ...
  componentWillMount() {
    if (this.props.post) {
      this.setState({
        ...
        if(this.props.post.postAuthor.authorType == 'user') {
          getUserById(this.props.post.postAuthor.authorID).then((res) => {
            this.setState({
              user: res.data
            })
          })
      ...
      return (
        <article className="article single">
          ...
          <div id="articleContent" className="content block">
            {this.state.isMounted && <InnerHTML html={post.postContent} /> }
          </div>
